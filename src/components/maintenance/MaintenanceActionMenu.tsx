...
export default function MaintenanceActionMenu({
  ...
  const hasActiveReminder = !task.reminderDisabled && (
    task.effectiveIntervalKm !== null
    || task.effectiveIntervalMonths !== null
    || task.conditionBasedDefault
  );
      ...
      <View className="flex-1 bg-black/55">
        <AppBottomSheet onClose={onClose} title={naturalMaintenanceActionLabel(task)}>
          <View className="gap-1">
            ...
            {customizable ? (
              <MenuRow
                icon={hasActiveReminder ? 'notifications-off' : 'notifications-active'}
                label={hasActiveReminder ? t('maintenance.disableReminder') : t('maintenance.enableReminder')}
                onPress={() => closeThen(() => onCustomize(task))}
                tone={hasActiveReminder ? 'warning' : 'default'}
              />
            ) : null}
