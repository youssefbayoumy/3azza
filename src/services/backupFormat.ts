import type { DatabaseBackupData } from './database';
import type {
  DocumentItem,
  GasLog,
  InventoryItem,
...
export type NormalizedBackupArchive = {
  source_schema: '3azza-local-backup/v1' | '3azza-local-backup/v2' | '3azza-local-backup/v3' | '3azza-local-backup/v4' | '3azza-local-backup/v5' | '3azza-local-backup/v6';
  data: DatabaseBackupData;
  document_files: EmbeddedDocumentFile[];
};
...
export function validateDatabaseBackupData(value: unknown): asserts value is DatabaseBackupData {
  ...
  profiles.forEach((profile, index) => {
    ...
    const selectionMode = profile.vehicle_selection_mode === undefined
      ? 'catalog'
      : requireString(profile, 'vehicle_selection_mode', path, { nonEmpty: true });
    if (selectionMode !== 'catalog' && selectionMode !== 'custom_brand') {
      invalid(`${path}.vehicle_selection_mode`, 'must be catalog or custom_brand');
    }
    const customBrandName = profile.custom_brand_name === undefined
      ? null
      : requireString(profile, 'custom_brand_name', path, { nullable: true });
    const customModelName = profile.custom_model_name === undefined
      ? null
      : requireString(profile, 'custom_model_name', path, { nullable: true });
    const capabilitiesVersion = requireNumber(
      profile,
      'vehicle_capabilities_version',
      path,
      { integer: true, min: 1 }
    );
    if (capabilitiesVersion !== VEHICLE_CAPABILITIES_SCHEMA_VERSION) {
      invalid(`${path}.vehicle_capabilities_version`, 'is not a supported capability schema');
    }
    const capabilitiesJson = requireString(
      profile,
      'vehicle_capabilities_json',
      path,
      { nonEmpty: true }
    ) as string;
    let capabilities: VehicleCapabilities | null;
    try {
      capabilities = vehicleCapabilitiesFromUnknown(JSON.parse(capabilitiesJson));
    } catch {
      capabilities = null;
    }
    if (!capabilities) {
      invalid(`${path}.vehicle_capabilities_json`, 'must contain a valid vehicle capability profile');
    }
    const hasAnyScooterField = scooterBrandId !== null || scooterModelId !== null || scooterVersionId !== null;
    if (selectionMode === 'catalog' && (customBrandName !== null || customModelName !== null)) {
      invalid(`${path}.custom_brand_name`, 'must be empty for a catalog vehicle');
    }
    if ((selectionMode === 'custom_brand' || hasAnyScooterField) && !resolveScooterSelection({
      selectionMode,
      ...
      invalid(`${path}.scooter_version_id`, 'must reference a complete catalog or custom vehicle identity');
    }
    requireDateTime(
...
function normalizePriorBackupData(data: PriorDatabaseBackupData): DatabaseBackupData {
  ...
  const vehicleProfiles: VehicleProfile[] = data.vehicle_profiles.map((vehicle) => ({
    ...
    vehicle_selection_mode: vehicle.vehicle_selection_mode ?? 'catalog',
    custom_brand_name: vehicle.custom_brand_name ?? null,
    custom_model_name: vehicle.custom_model_name ?? null,
    vehicle_capabilities_version: VEHICLE_CAPABILITIES_SCHEMA_VERSION,
    vehicle_capabilities_json: serializeVehicleCapabilities(
...
export function normalizeBackupArchive(value: unknown): NormalizedBackupArchive {
  ...
  if (value.schema === '3azza-local-backup/v6' && isObject(value.data)) {
    requireBackupArrays(value.data);
    validateDatabaseBackupData(value.data);
    ...
    return {
      source_schema: '3azza-local-backup/v6',
      data,
      document_files: value.document_files,
    };
  }

  if (value.schema === '3azza-local-backup/v5' && isObject(value.data)) {
    requireBackupArrays(value.data);
    const data = normalizePriorBackupData(value.data as PriorDatabaseBackupData);
    validateDatabaseBackupData(data);
    validateEmbeddedDocumentFiles(value.document_files, data);
    return {
      source_schema: '3azza-local-backup/v5',
      data,
      document_files: value.document_files,
    };
  }
