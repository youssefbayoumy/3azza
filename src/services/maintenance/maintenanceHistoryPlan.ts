import type { MaintenanceAction, ScooterMaintenanceProfile } from '../../maintenance/types';
...
export function maintenanceHistoryBaselineKeysForProfile(
  profile: ScooterMaintenanceProfile
): MaintenanceHistoryBaselineKey[] {
  const hasComponent = (componentId: string) => profile.rules.some(
    (rule) => rule.applicable && rule.componentId === componentId
  );
  const keys: MaintenanceHistoryBaselineKey[] = [];
  if (hasComponent('engine-oil')) keys.push('engine_oil');
  if (hasComponent('transmission-oil')) keys.push('transmission_oil');
  if (hasComponent('air-cleaner-element')) keys.push('air_filter');
  keys.push('general_inspection');
  return keys;
}
