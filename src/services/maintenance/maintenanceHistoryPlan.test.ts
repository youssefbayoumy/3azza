...
import { getUniversalCustomMaintenanceProfile } from '../../maintenance/universalProfile';
...
describe('maintenance-history onboarding plan', () => {
  ...
  it('shows onboarding baselines only for applicable vehicle systems', () => {
    const electric = getUniversalCustomMaintenanceProfile({
      schemaVersion: 1,
      powertrain: 'electric',
    ...
    assert.deepEqual(maintenanceHistoryBaselineKeysForProfile(electric), ['general_inspection']);

    const combustion = getUniversalCustomMaintenanceProfile({
      schemaVersion: 1,
    ...
    assert.deepEqual(maintenanceHistoryBaselineKeysForProfile(combustion), [
      'engine_oil',
      'transmission_oil',
      'air_filter',
      'general_inspection',
